...
// 
// Related Topics Array Greedy Heap (Priority Queue) 👍 2987 👎 71
 ...
 * 
 * @author: eumes
 * @date: 2022-06-22 00:10:40
 */
 public class P1642FurthestBuildingYouCanReach {
    public static void main(String[] args) {
//        Solution_P1642 solution = new Solution_P1642();
        // TO TEST
...
class Solution_P1642 {
    public int furthestBuilding(int[] heights, int bricks, int ladders) {
        // 小顶堆，存储需要使用ladder的间隔
        PriorityQueue<Integer> heap = new PriorityQueue<>();
        for (int i = 0; i < heights.length - 1; i++) {
            int diff = heights[i + 1] - heights[i];
            // diff > 0，需要使用工具，加入heap
            if (diff > 0) {
                heap.offer(diff);
            }
            // 如果heap中的数量 > ladder，说明需要使用brick；此时使用最少的brick
            if (heap.size() > ladders) {
                bricks -= heap.poll();
            }
            // 如果此时brick < 0，说明工具使用完了
            if (bricks < 0) {
                return i;
            }
        }
